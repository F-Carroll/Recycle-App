...
app.put("/api/items/:id", async(req,res) => {
    try {
       ...
       const barcode = req.body.barcode;
       const items = req.body.items;
       const updateProduct = await pool.query("UPDATE products SET barcode = $1, items = $2 WHERE product_id = $3",
       [barcode, items, id]
       );
...
app.delete("/api/items/:id", async (req, res) => {
    try {
        const {id} = req.params;
        const deleteProduct = await pool.query("DELETE FROM products WHERE product_id = $1",
        [id]
        );
...
app.get("/api/locations/:id", async (req, res) => {
    try {
        const {id} = req.params;
        const location = await pool.query("SELECT * FROM locations WHERE location_id = $1",
        [id]
        );
...
app.put("/api/locations/:id", async(req,res) => {
    try {
       ...
       const location_name = req.body.location_name;
       const location_materials = req.body.location_materials;
       const updateLocation = await pool.query("UPDATE locations SET location_name = $1, location_materials = $2 WHERE location_id = $3",
...
app.delete("/api/locations/:id", async (req, res) => {
    try {
        const {id} = req.params;
        const deleteProduct = await pool.query("DELETE FROM locations WHERE location_id = $1",
        [id]
        );
