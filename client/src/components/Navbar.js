import React, {useEffect} from "react";
import {
  Flex,
  Center,
  Spacer,
  Button,
  Modal,
  ModalOverlay,
  ModalContent,
  ModalHeader,
  ModalFooter,
  ModalBody,
  ModalCloseButton,
  useDisclosure,
  Heading,
  Box,
  Text,
  Link,
} from "@chakra-ui/react";
import LocationSelector from "./LocationSelector";

export default function Navbar() {
  const { isOpen, onOpen, onClose } = useDisclosure();

function openSettings() {
  window.location = "./settings"
}

  function AddSvg() {
    return (
      <svg
        id="a14aeead-7a51-49c7-9dde-c662f5733ecb"
        data-name="Layer 1"
        xmlns="http://www.w3.org/2000/svg"
        width="80%"
        height="80%"
        viewBox="0 0 782.04441 701.88002"
      >
        <path
          d="M609.48783,100.59015l-25.44631,6.56209L270.53735,187.9987,245.091,194.56079A48.17927,48.17927,0,0,0,210.508,253.17865L320.849,681.05606a48.17924,48.17924,0,0,0,58.61776,34.58317l.06572-.01695,364.26536-93.93675.06572-.01695a48.17923,48.17923,0,0,0,34.58309-58.6178l-110.341-427.87741A48.17928,48.17928,0,0,0,609.48783,100.59015Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#f2f2f2"
        />
        <path
          d="M612.94784,114.00532l-30.13945,7.77236L278.68955,200.20385l-30.139,7.77223a34.30949,34.30949,0,0,0-24.6275,41.74308l110.341,427.87741a34.30946,34.30946,0,0,0,41.7431,24.62736l.06572-.01695,364.26536-93.93674.06619-.01707a34.30935,34.30935,0,0,0,24.627-41.7429l-110.341-427.87741A34.30938,34.30938,0,0,0,612.94784,114.00532Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#fff"
        />
        <path
          d="M590.19,252.56327,405.917,300.08359a8.01411,8.01411,0,0,1-4.00241-15.52046l184.273-47.52033A8.01412,8.01412,0,0,1,590.19,252.56327Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#f2f2f2"
        />
        <path
          d="M628.955,270.49906,412.671,326.27437a8.01411,8.01411,0,1,1-4.00241-15.52046l216.284-55.77531a8.01411,8.01411,0,0,1,4.00242,15.52046Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#f2f2f2"
        />
        <path
          d="M620.45825,369.93676l-184.273,47.52032a8.01411,8.01411,0,1,1-4.00242-15.52046l184.273-47.52032a8.01411,8.01411,0,1,1,4.00241,15.52046Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#f2f2f2"
        />
        <path
          d="M659.22329,387.87255l-216.284,55.77531a8.01411,8.01411,0,1,1-4.00242-15.52046l216.284-55.77531a8.01411,8.01411,0,0,1,4.00242,15.52046Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#f2f2f2"
        />
        <path
          d="M650.72653,487.31025l-184.273,47.52033a8.01412,8.01412,0,0,1-4.00242-15.52047l184.273-47.52032a8.01411,8.01411,0,0,1,4.00242,15.52046Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#f2f2f2"
        />
        <path
          d="M689.49156,505.246l-216.284,55.77532a8.01412,8.01412,0,1,1-4.00241-15.52047l216.284-55.77531a8.01411,8.01411,0,0,1,4.00242,15.52046Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#f2f2f2"
        />
        <path
          d="M374.45884,348.80871l-65.21246,16.817a3.847,3.847,0,0,1-4.68062-2.76146L289.5963,304.81607a3.847,3.847,0,0,1,2.76145-4.68061l65.21247-16.817a3.847,3.847,0,0,1,4.68061,2.76145l14.96947,58.04817A3.847,3.847,0,0,1,374.45884,348.80871Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M404.72712,466.1822l-65.21247,16.817a3.847,3.847,0,0,1-4.68062-2.76146l-14.96946-58.04816A3.847,3.847,0,0,1,322.626,417.509l65.21246-16.817a3.847,3.847,0,0,1,4.68062,2.76145l14.96946,58.04817A3.847,3.847,0,0,1,404.72712,466.1822Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M434.99539,583.55569l-65.21246,16.817a3.847,3.847,0,0,1-4.68062-2.76145l-14.96946-58.04817a3.847,3.847,0,0,1,2.76145-4.68062l65.21247-16.817a3.847,3.847,0,0,1,4.68061,2.76146l14.96947,58.04816A3.847,3.847,0,0,1,434.99539,583.55569Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M863.63647,209.0517H487.31811a48.17928,48.17928,0,0,0-48.125,48.12512V699.05261a48.17924,48.17924,0,0,0,48.125,48.12507H863.63647a48.17924,48.17924,0,0,0,48.125-48.12507V257.17682A48.17928,48.17928,0,0,0,863.63647,209.0517Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M863.637,222.90589H487.31811a34.30948,34.30948,0,0,0-34.271,34.27093V699.05261a34.30947,34.30947,0,0,0,34.271,34.27088H863.637a34.30936,34.30936,0,0,0,34.27051-34.27088V257.17682A34.30937,34.30937,0,0,0,863.637,222.90589Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#fff"
        />
        <circle cx="694.19401" cy="614.02963" r="87.85039" fill="#39F969" />
        <path
          d="M945.18722,701.63087H914.63056V671.07421a11.45875,11.45875,0,0,0-22.9175,0v30.55666H861.1564a11.45875,11.45875,0,0,0,0,22.9175h30.55666V755.105a11.45875,11.45875,0,1,0,22.9175,0V724.54837h30.55666a11.45875,11.45875,0,0,0,0-22.9175Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#fff"
        />
        <path
          d="M807.00068,465.71551H616.699a8.01412,8.01412,0,1,1,0-16.02823H807.00068a8.01412,8.01412,0,0,1,0,16.02823Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M840.05889,492.76314H616.699a8.01412,8.01412,0,1,1,0-16.02823H840.05889a8.01411,8.01411,0,1,1,0,16.02823Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M807.00068,586.929H616.699a8.01412,8.01412,0,1,1,0-16.02823H807.00068a8.01411,8.01411,0,0,1,0,16.02823Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M840.05889,613.97661H616.699a8.01412,8.01412,0,1,1,0-16.02823H840.05889a8.01412,8.01412,0,1,1,0,16.02823Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M574.07028,505.04162H506.72434a3.847,3.847,0,0,1-3.84278-3.84278V441.25158a3.847,3.847,0,0,1,3.84278-3.84278h67.34594a3.847,3.847,0,0,1,3.84278,3.84278v59.94726A3.847,3.847,0,0,1,574.07028,505.04162Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M574.07028,626.25509H506.72434a3.847,3.847,0,0,1-3.84278-3.84278V562.46505a3.847,3.847,0,0,1,3.84278-3.84278h67.34594a3.847,3.847,0,0,1,3.84278,3.84278v59.94726A3.847,3.847,0,0,1,574.07028,626.25509Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#e6e6e6"
        />
        <path
          d="M807.21185,330.781H666.91017a8.01411,8.01411,0,0,1,0-16.02823H807.21185a8.01411,8.01411,0,0,1,0,16.02823Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#ccc"
        />
        <path
          d="M840.27007,357.82862H666.91017a8.01411,8.01411,0,1,1,0-16.02822h173.3599a8.01411,8.01411,0,0,1,0,16.02822Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#ccc"
        />
        <path
          d="M635.85911,390.6071H506.51316a3.847,3.847,0,0,1-3.84277-3.84277V285.81706a3.847,3.847,0,0,1,3.84277-3.84277H635.85911a3.847,3.847,0,0,1,3.84277,3.84277V386.76433A3.847,3.847,0,0,1,635.85911,390.6071Z"
          transform="translate(-208.9778 -99.05999)"
          fill="#39F969"
        />
      </svg>
    );
  }
  function AddLocation() {
    return (
      <svg
        id="af8e4536-3e0b-4cf9-a0b4-bd60479f2728"
        data-name="Layer 1"
        xmlns="http://www.w3.org/2000/svg"
        width="80%"
        height="80%"
        viewBox="0 0 949.72035 767.83442"
      >
        <ellipse
          cx="738.31694"
          cy="538.00578"
          rx="48.50275"
          ry="98.96521"
          fill="#3f3d56"
        />
        <path
          d="M858.97775,766.07866c19.42586-116.55145.19557-232.6818-.00094-233.8403l-3.77868.64008c.19558,1.15288,19.31544,116.6731-.00094,232.57138Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#39F969"
        />
        <rect
          x="839.86385"
          y="571.35817"
          width="3.83318"
          height="49.73959"
          transform="translate(-206.13169 991.20198) rotate(-61.8584)"
          fill="#39F969"
        />
        <rect
          x="863.20238"
          y="606.06946"
          width="49.7413"
          height="3.83318"
          transform="translate(-306.94737 424.95757) rotate(-28.15796)"
          fill="#39F969"
        />
        <ellipse
          cx="854.85467"
          cy="381.37918"
          rx="94.86568"
          ry="193.56431"
          fill="#e6e6e6"
        />
        <path
          d="M969.42791,763.99514c37.944-227.6604.38273-454.4917-.00094-456.75629l-3.77868.64007c.38273,2.259,37.8336,228.48015-.00094,455.48737Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#39F969"
        />
        <rect
          x="935.68168"
          y="383.44336"
          width="3.83318"
          height="97.28714"
          transform="translate(-10.7695 988.96912) rotate(-61.8584)"
          fill="#39F969"
        />
        <rect
          x="979.49707"
          y="453.16808"
          width="97.28714"
          height="3.83318"
          transform="translate(-218.21638 472.94968) rotate(-28.15742)"
          fill="#39F969"
        />
        <ellipse
          cx="630.17281"
          cy="264.87747"
          rx="129.81619"
          ry="264.87747"
          fill="#e6e6e6"
        />
        <path
          d="M740.15609,763.99514c51.904-311.42024.524-621.7035-.00093-624.80093l-3.77869.64007c.524,3.09182,51.79358,312.765-.00093,623.532Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#39F969"
        />
        <rect
          x="695.37982"
          y="243.35561"
          width="3.83318"
          height="133.12944"
          transform="translate(-30.00782 712.52818) rotate(-61.8584)"
          fill="#39F969"
        />
        <rect
          x="754.63183"
          y="339.47403"
          width="133.12944"
          height="3.83318"
          transform="translate(-189.05666 361.82738) rotate(-28.15671)"
          fill="#39F969"
        />
        <polygon
          points="910.513 700.231 0 700.231 0 698.05 910.908 698.05 910.513 700.231"
          fill="#3f3d56"
        />
        <path
          d="M281.13989,742H259.13977c-11.50788,0-21.78809.81433-28.57416,5.17487A36.9869,36.9869,0,0,0,198.13989,728H176.13977a36.99993,36.99993,0,0,0-36.99994,36.99994V765h179v-.00006C318.13983,744.56543,301.5744,742,281.13989,742Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#3f3d56"
        />
        <path
          d="M881.1535,766.08279h93.97265c-.47461-17.53223-13.60546-21-35.98632-21h-22C894.759,745.08279,881.62811,748.55056,881.1535,766.08279Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#3f3d56"
        />
        <path
          d="M318.357,505.91951h14a0,0,0,0,1,0,0v21a7,7,0,0,1-7,7h0a7,7,0,0,1-7-7v-21a0,0,0,0,1,0,0Z"
          transform="translate(123.33 -154.65474) rotate(24.83609)"
          fill="#ffb9b9"
        />
        <path
          d="M324.21715,171.82885h.0001A50.99995,50.99995,0,0,1,375.2172,222.8288v57.00005a0,0,0,0,1,0,0h-102a0,0,0,0,1,0,0V222.8288a50.99995,50.99995,0,0,1,50.99995-50.99995Z"
          fill="#2f2e41"
        />
        <polygon
          points="286.217 708.837 275.217 706.837 267.217 662.837 289.717 663.329 286.217 708.837"
          fill="#ffb9b9"
        />
        <polygon
          points="407.717 734.329 396.437 739.542 378.437 687.542 397.717 684.329 407.717 734.329"
          fill="#ffb9b9"
        />
        <path
          d="M527.857,753.41165l-29.5,1.50786-80.5-194.50786-2.19541,175.16761L387.357,731.91951c-19.95624-109.01056-36.468-205.84771-10.5-266.50786l88,7Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#2f2e41"
        />
        <path
          d="M393.67344,805.57244l-11.44751-.61879a2.95914,2.95914,0,0,1-2.41814-4.4078l17.42365-30.91294,15.30047,1.275,2.36175,11.02149a11.17331,11.17331,0,0,1-4.03429,11.13641l-7.503,5.87854A9.64078,9.64078,0,0,1,393.67344,805.57244Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#2f2e41"
        />
        <path
          d="M549.64469,830.30007l-9.88787,3.399a4.016,4.016,0,0,1-3.90262-.73465l-17.21007-14.59114a10.65761,10.65761,0,0,1-3.29419-11.26339l2.16141-7.02455,16.38531-3.17135,17.80971,27.39954A4.016,4.016,0,0,1,549.64469,830.30007Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#2f2e41"
        />
        <circle cx="317.2172" cy="217.82885" r="34" fill="#ffb9b9" />
        <polygon
          points="351.717 268.329 311.717 275.329 307.717 243.329 338.717 239.329 351.717 268.329"
          fill="#ffb9b9"
        />
        <path
          d="M476.857,483.41165l-103-10c5.05072-54.95356,21.97741-103.41947,57-142l50-4Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#3f3d56"
        />
        <polygon
          points="214.217 447.837 193.217 440.837 229.457 367.57 283.717 279.329 305.717 265.329 257.041 374.412 214.217 447.837"
          fill="#3f3d56"
        />
        <polygon
          points="340.217 206.829 278.217 218.829 288.217 187.829 324.217 179.829 352.217 189.829 340.217 206.829"
          fill="#2f2e41"
        />
        <circle cx="212.08329" cy="364.86298" r="93.69065" fill="#39F969" />
        <polygon
          points="320.426 471.91 263.743 457.087 207.059 442.263 248.238 400.586 289.418 358.908 304.922 415.409 320.426 471.91"
          fill="#39F969"
        />
        <circle cx="212.19834" cy="363.91447" r="33.61481" fill="#fff" />
        <path
          d="M430.357,528.91951h14a0,0,0,0,1,0,0v21a7,7,0,0,1-7,7h0a7,7,0,0,1-7-7v-21a0,0,0,0,1,0,0Z"
          transform="translate(143.34908 -199.5702) rotate(24.83609)"
          fill="#ffb9b9"
        />
        <path
          d="M450.357,539.91951l-18-11,9.43849-64.247L480.857,332.41165l7.4185,2.83648A31.91285,31.91285,0,0,1,507.792,372.979l-25.31459,98.77653Z"
          transform="translate(-125.13983 -66.08279)"
          fill="#3f3d56"
        />
      </svg>
    );
  }

  return (
    <div>
      <Flex
        borderLeftWidth="4px"
        borderLeftColor="#39F969"
        boxShadow="md"
        h="60px"
        mb="15px"
      >
        <Center>
          <Button mx="10px" h="35px">
            Logo
          </Button>
        </Center>

        <Spacer />
        
        <Center>
          <Button ml="10px" p="0" onClick={onOpen}>
          <svg height="23px"className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
          </Button>
        </Center>
        <Center>
          <Button mx="10px" p="0" onClick={openSettings}>
            <svg
              height="23px"
              className="w-6 h-6"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"
              />
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"
              />
            </svg>
          </Button>
        </Center>

        <Modal size="xl" isOpen={isOpen} onClose={onClose}>
          <ModalOverlay />
          <ModalContent mt={["40%", "10%"]} mx="15px" borderRadius="none">
            <ModalHeader>What do you want to add?</ModalHeader>
            <ModalCloseButton />
            <ModalBody>
              <Link href="/add/item">
                <Flex
                  color="black"
                  h="110px"
                  bg="#F0F0F0"
                  borderRadius="5px"
                  cursor="pointer"
                  mb="15px"
                >
                  <Center w="30%">
                    <AddSvg />
                  </Center>
                  <Box flex="1" my="auto" ml="1vw">
                    <Heading size="md">Add Item</Heading>
                    <Text>Add another item to the database</Text>
                  </Box>
                </Flex>
              </Link>
              <Link href="/add/location">
                <Flex color="black" h="110px" bg="#F0F0F0" borderRadius="5px">
                  <Center w="30%">
                    <AddLocation />
                  </Center>
                  <Box flex="1" my="auto" ml="1vw">
                    <Heading size="md">Add Location</Heading>
                    <Text>Add a new recycling location to the database</Text>
                  </Box>
                </Flex>
              </Link>
            </ModalBody>

            <ModalFooter>
              <Button colorScheme="blue" onClick={onClose}>
                Cancel
              </Button>
            </ModalFooter>
          </ModalContent>
        </Modal>
        
      </Flex>
    </div>
  );
}
